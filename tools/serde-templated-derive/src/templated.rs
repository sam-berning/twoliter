...
impl TemplatedOpts {
    pub(crate) fn templated_struct_def(&self) -> Result<proc_macro2::TokenStream> {
        ...
        let generated_struct_name = format_ident!("Templated{}", ident);
    ...
    pub(crate) fn render_template_of_trait_def(&self) -> Result<proc_macro2::TokenStream> {
        ...
        let render_assignments: Vec<proc_macro2::TokenStream> = self
            .defined_fields()?
            .iter()
            .map(|field| field.render_assignment())
            .collect::<Result<_, _>>()?;
    ...
    fn generate_templated_field(&self) -> Result<proc_macro2::TokenStream> {
        ...
        let final_template_type: proc_macro2::TokenStream =
            if let Some(template_as) = template_as.as_ref() {
                ...
                // For generic structs, we're replacing the leaf types
                // e.g.
                ...
                visit_generic_leaves_of_type(&mut modified_type, &mut |leaf_type| {
...
/// Calls the input `visitor` function for all "leaves" of a generic type path.
///
/// For examples of what we consider leaves:
/// * Option<String> -> `String` is the leaf type
/// * Result<T, E> -> `T` and `E` are leaf types
/// * Option<Result<T, E>> -> `T` and `E` are leaf types
fn visit_generic_leaves_of_type(ty: &mut syn::Type, visitor: &mut impl FnMut(&mut syn::Type)) {
            ...
            if let syn::PathArguments::AngleBracketed(generic_args) = &mut seg.arguments {
                for generic_arg in &mut generic_args.args {
                    if let syn::GenericArgument::Type(inner_ty) = generic_arg {
                        visit_generic_leaves_of_type(inner_ty, visitor);
                    }
                }
            } else {
                visitor(ty);
